  ...
  constructor() {
    ...
    this.version(10)
      .stores({
        ...
        settings:
          "++id, notifications, todayNotifications, notificationTime, uuid, user_uuid",
      })
      .upgrade((transaction) => {
        try {
          ...
          const serialFns = {
            createUser: () =>
              transaction.table<UserDto, number>("user").add(
                {
                  uuid: randomUUID(),
                  email: "",
                  name: "",
            ...
            getUser: (userId: number) =>
              transaction.table<UserDto>("user").get(userId),
            addUuidToItems: (user: UserDto) =>
              Promise.all([
                transaction
                  .table<TaskDto, number>("task")
                  .toCollection()
                  .modify((item) => {
                    if (!item.uuid) {
                ...
                transaction
                  .table<ProjectDto, number>("project")
                  .toCollection()
                  .modify((item) => {
                    if (!item.uuid) {
                      item.uuid = randomUUID();
                ...
                transaction
                  .table<SettingsDto, number>("settings")
                  .toCollection()
                  .modify((item) => {
                    if (!item.uuid) {
                      item.uuid = randomUUID();
            ...
            getProjects: () =>
              transaction.table<ProjectDto, number>("project").toArray(),
            getProjectMap: (projects: ProjectDto[]): Map<number, string> => {
              return new Map<number, string>(
                projects.map((project) => [project.id, project.uuid]),
              );
            },
            getTaskMap: (projectMap: Map<number, string>) =>
              transaction
                .table<TaskDto>("task")
                .toArray()
                .then((tasks) => {
                  return [
                    new Map<number, string>(
                      tasks.map((task) => [task.id, task.uuid]),
                    ),
                    projectMap,
                  ];
                }),
            getTagMap: ([tasksMap, projectsMap]: Map<number, string>[]) =>
              transaction
                .table<TagDto>("tag")
                .toArray()
                .then((tags) => {
                  return [
                    ...
                    projectsMap,
                    new Map<number, string>(
                      tags.map((tag) => [tag.id, tag.uuid]),
                    ),
                  ];
                }),
            addForeignKeysUUID: ([tasksMap, projectsMap, tagsMap]: Map<
              number,
              string
            >[]) =>
              Promise.all([
                transaction
                  ...
                  .toCollection()
                  .modify((item: TaskDto) => {
                    item.project_uuid = item.project
                      ? (projectsMap.get(item.project) as string)
                      : "";
                    item.parent_uuid = item.parent
                      ? (tasksMap.get(item.parent) as string)
                      : "";
                  }),
                transaction
                  .table("task_tag")
                  .toCollection()
                  .modify((item: TaskTagDto) => {
                    item.task_uuid = item.task
                      ? (tasksMap.get(item.task) as string)
                      : "";
                    item.tag_uuid = item.tag
                      ? (tagsMap.get(item.tag) as string)
                      : "";
                  }),
              ]),
          };

          Object.values(serialFns).reduce((promise, nextFunction) => {
            return promise.then((previousPromiseResult) =>
              runTaskSerial(nextFunction, previousPromiseResult),
            );
          }, Promise.resolve());
        } catch (error) {
          transaction.abort();
    ...
    this.version(12).stores({
      ...
      settings:
        "++id, notifications, todayNotifications, notificationTime, uuid, user_uuid, updated_at",
    });
    ...
    this.version(13)
      .stores({
        ...
        // task: null,
        task_temp:
          "$$uuid, title, description, dueDate, dueTime, project, completed, order, parent, recurring, user_uuid, project_uuid, parent_uuid, updated_at",
        // project: null,
        ...
        // settings: null,
        settings_temp:
          "$$uuid, notifications, todayNotifications, notificationTime, user_uuid, updated_at",
      })
      .upgrade(async (transaction) => {
        const tasks = (await transaction.table("task").toArray()).map(
          (item) => {
            const newItem: Record<string, any> = {};
            Object.keys(item).forEach((key) =>
              key !== "id" ? (newItem[key] = item[key]) : null,
            );
            return newItem;
          },
        );
        ...
        const projects = (await transaction.table("project").toArray()).map(
          (item) => {
            const newItem: Record<string, any> = {};
            Object.keys(item).forEach((key) =>
              key !== "id" ? (newItem[key] = item[key]) : null,
        ...
        const settings = (await transaction.table("settings").toArray()).map(
          (item) => {
            const newItem: Record<string, any> = {};
            Object.keys(item).forEach((key) =>
              key !== "id" ? (newItem[key] = item[key]) : null,
    ...
    this.version(17).stores({
      ...
      settings:
        "$$uuid, id, notifications, todayNotifications, notificationTime, user_uuid, updated_at",
    });
    ...
    this.version(20).stores({
      settings:
        "$$uuid, id, notifications, todayNotifications, notificationTime, user_uuid, updated_at, autostart",
    });
    ...
    this.version(25).stores({
      task_attachment:
        "$$uuid, task_uuid, user_uuid, blob, updated_at, name, file_type",
    });
